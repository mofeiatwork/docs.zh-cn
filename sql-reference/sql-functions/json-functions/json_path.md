...
查询JSON对象的语言。当前Starrocks实现了SQL/JSONPath语法子集，支持的语法如下。

- 嵌套对象查询: `$.a.b.c`, 可以查询嵌套多层的JSON对象
- 数组元素查询：`$.a[1]`, 可以查询某个数组元素
- 数组片段：`$.a[1:3]`, 查询数组中的多个元素
- 数组提取：`$.a[*].b`，可以提取数组中的对象

## 注意事项

当前实现的JSON Path没有完全遵循SQL/JSONPath标准，明确支持的语法可参考此文档，其余语法不支持。
